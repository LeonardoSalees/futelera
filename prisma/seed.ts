...
async function main() {
  ...
  // 1. Primeiro criamos os times vinculados àquele dia de jogo
  const timeA = await prisma.team.create({
    data: {
      name: "Os de Colete",
      matchDayId: matchDay.id, // matchDay criado anteriormente no seed
      players: {
        connect: players.slice(0, 5).map((p) => ({ id: p.id })), // Conecta os 5 primeiros jogadores
      },
    },
  });

  const timeB = await prisma.team.create({
    data: {
      name: "Sem Colete",
      matchDayId: matchDay.id,
      players: {
        connect: players.slice(5, 10).map((p) => ({ id: p.id })), // Conecta os próximos 5
      },
    },
  });

  // 2. Agora criamos a partida conectando esses times
  const match = await prisma.match.create({
    data: {
      ...
      status: "finished",
      teams: {
        connect: [{ id: timeA.id }, { id: timeB.id }],
      },
    },
  });
