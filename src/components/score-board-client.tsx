...
export function ScoreboardClient({
  ...
  const handleFinalizeGoal = async (assistantId: string | null) => {
    ...
    const assistantObj = assistantId
      ? currentPlayers.find((p: any) => p.id === assistantId)
      : null;
    ...
    try {
      ...
      if (res.ok) {
        ...
        const goalWithNames = {
          ...
          assistant:
            newGoal.assistant ||
            (assistantObj ? { name: assistantObj?.name } : null),
        };
  ...
  if (!confirm("Encerrar este confronto e salvar o placar final?")) return;

  try {
    // Note que aqui o ID é o da MATCH (confronto), não da MatchDay
    const res = await fetch(`/api/matches/${matchId}/finish`, { 
      method: "POST" 
    });

    if (res.ok) {
      router.refresh(); // Invalida o cache para o card sumir da Home
      router.push(`/partida/${latestMatchday.id}`); // Volta para a Home
    }
  } catch (e) {
    console.error(e);
  }
  ...
  const currentPlayers =
    goalProcess.team === "A" ? teamA?.players : teamB?.players;
    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 p-3 flex flex-col gap-4 select-none">
      ...
      <Card className="max-w-2xl mx-auto w-full bg-gradient-to-b from-slate-900 to-black border-white/10 rounded-[2.5rem] shadow-2xl overflow-hidden mt-8">
        <div className="p-6 space-y-8">
          <div className="flex flex-col items-center">
            <div className="flex gap-2 mb-4">
              <Button
                onClick={toggleTimer}
                className={cn(
                  "h-10 px-6 rounded-xl font-black uppercase italic text-xs",
                  isActive ? "bg-rose-600" : "bg-blue-600",
                )}
              >
            ...
            <div
              className={cn(
                "text-7xl font-[1000] italic font-mono leading-none tracking-tighter",
          ...
          <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-4 pt-4 border-t border-white/5">
            ...
            />
            <div className="text-zinc-800 font-black italic text-2xl px-2">
              VS
            </div>
            <TeamDisplay
              name={teamB?.name}
          ...
          {sortedTimeline.length === 0 ? (
            ...
            sortedTimeline.map((goal: any) => (
              ...
              >
                <div
                  className={cn(
                    "text-2xl font-[1000] italic",
                    goal.teamSide === "A" ? "text-blue-500" : "text-amber-500",
                  )}
                >
                  {goal.minute}'
                </div>
                <div className="flex-1">
                  <div
                    className={cn(
                      ...
                      goal.teamSide === "B" && "flex-row-reverse",
                    )}
                  >
                    <Zap
                      size={14}
                  ...
                  {goal.assistant?.name && (
                    <p
                      className={cn(
                        "text-[10px] font-bold text-slate-400 mt-0.5 uppercase",
                        goal.teamSide === "B" && "mr-1",
                      )}
                    >
                      Garçom:{" "}
      ...
      <div className="max-w-2xl mx-auto w-full mb-10">
        <Button
          ...
          className="w-full h-14 bg-white text-black hover:bg-slate-200 rounded-2xl font-black uppercase italic"
        >
          Finalizar Partida
        </Button>
      </div>
      ...
      {/* DIALOG DE SELEÇÃO */}
      <Dialog
        open={goalProcess.open}
        onOpenChange={(o) => setGoalProcess((p) => ({ ...p, open: o }))}
      >
        <DialogContent className="bg-[#020617] border-white/10 text-white max-w-md rounded-[2rem]">
          ...
          <div className="grid gap-2 mt-4 max-h-[50vh] overflow-y-auto">
            {currentPlayers?.map((player: any) => (
              <button
                ...
                disabled={
                  ...
                  player.id === goalProcess.scorerId
                }
                onClick={() =>
                  goalProcess.step === "scorer"
                    ? setGoalProcess((p) => ({
                        ...p,
                        scorerId: player.id,
                        step: "assistant",
            ...
            {goalProcess.step === "assistant" && (
              <Button
                onClick={() => handleFinalizeGoal(null)}
                variant="ghost"
                className="text-slate-500 font-black uppercase text-xs mt-2"
              >
                Sem Assistência
              </Button>
            )}
...
function TeamDisplay({ name, score, color, isActive, onGoal }: any) {
  return (
    <div className="flex flex-col items-center gap-3 overflow-hidden">
      <h2
        className={cn(
          "text-[10px] font-black italic uppercase truncate w-full text-center px-1",
          color,
        )}
      >
        {name || "Time"}
